...
% -= Lie =-
\chapter{Le groupe $SO(n)$ et son algèbre de Lie $\mathfrak{so}(n)$}
...
\subsection{Propriétés de l'action du groupe $SO(n)$ sur $\mathbb{R}^n$}
...
\subsection{Propriétés topologiques de $SO(n)$}
...
% Algèbre de Lie so(n)
\section{L'algèbre de Lie $\mathfrak{so}(n)$}
...
% -= Equa diff =-
\chapter{Étude de l'équation différentielle linéaire $\dot{x}(t)=Ax(t)$}
...
\section{Obtenir l'équation différentielle $\dot{x}(t)=Ax(t)$}
...
\section{Solution de $\dot{x}(t)=Ax(t)$}
...
\chapter{Cas particuliers de $SO(2)$ et $SO(3)$}\label{chap:SO2-SO3}
...
\section{$SO(2)$, Les rotations en dimension 2}
...
\subsection{Recalage rigide sur $SO(2)$}
...
\begin{figure}[H]\label{fig:RecalageRotationSO2}
  \centering
  ...
  \caption{Première et $50^{\text{ième}}$ étape du recalage sur les rotations $SO(2)$.}
\end{figure}
\end{exemple}
