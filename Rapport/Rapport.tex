...
Pour l'équation sur $p$, on dérive le Hamiltonien réduit $H_r(q,p)=H(q,p,A^*(q,p))$ par rapport à $q$. Comme $A^*(q,p)$ maximise $H(q,p,\cdot)$ sur $\mathfrak{so}(n)$, la dérivée de $A^*$ par rapport à $q$ s'annule et il reste donc seulement $H$ exprimé en fonction de $q$:
\[
