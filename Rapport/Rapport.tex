...
Dans la formulation géométrique du recalage, on considère un espace de champs de vecteurs $V\hookrightarrow C^{2}_{0}(\mathbb{R}^d,\mathbb{R}^d)$ et un espace de formes $Q$. Cet espace peut être, selon le problème étudié, un espace d'images, de courbes, de surfaces ou encore de nuages de points. Dans le cas des nuages de points, l'espace des formes est
\[
Q={(\mathbb{R}^d)}^N.
\]
...
Le problème de recalage qui consiste à trouver la meilleure déformation peut s'écrire comme un problème variationnel où l'on doit déterminer le champ de vitesse $v_t$ qui minimise une énergie composée de deux termes:
...
\partial_v H(q,p,v)=0.
\]
On obtient ainsi un champ de vecteurs optimal
\[
v^*=v^*(q,p).
\]
...
\begin{proposition}
L'action naturelle de $SO(n)$ sur $\mathbb R^n$ est fidèle, mais elle n'est ni libre ni transitive
\end{proposition}
...
Elle n'est pas libre sur $\mathbb R^n$, puisque $R0=0$ pour tout $R\in SO(n)$.\\
Sur $\mathbb R^2\setminus\{0\}$, si une rotation fixe un vecteur non nul, son angle est nul modulo $2\pi$, donc elle est égale à l'identité.\\
En revanche, pour $n\geq3$, choisissons
\[
R=\begin{pmatrix}1&0\\0&S\end{pmatrix}, \qquad S\in SO(n-1),\quad S\neq I_{n-1}.
\]
Alors $R\neq I_n$ mais $Re_1=e_1$. Donc \underline{l'action de $SO(n)$ dans $\mathbb{R}^n$ n'est pas libre}.\\
...
\begin{proof}
Le groupe $SO(n)$ est compact~(\ref{prop:SO-compact}) et c'est aussi une variété différentielle~(\ref{prop:SO-groupe-de-Lie}). Donc, il est donc localement connexe par arcs et possède un nombre finie de composantes connexes par arcs.\\
Il ne reste plus qu'à prouver qu'il n'y a qu'une unique composante:\\
Notons $C$ la composante connexe par arcs de l'identité $I_n$ dans $SO(n)$. Puisque $SO(n)$ est une variété différentielle, il existe $U$ un voisinage ouvert de $I_n$ qui est homéomorphe à un ouvert de $\mathbb{R}^{n(n-1)/2}$. On peut choisir $U$ connexe par arcs, de sorte que $U\subset C$.\\
Par ailleurs, pour toute matrice $A\in SO(n)$, la translation à gauche
\[
\begin{aligned}
L_A\colon SO(n)&\longrightarrow SO(n)\\
M&\longmapsto AM
\end{aligned}
\]
est un homéomorphisme. Elle envoie donc la composante $C$ de $I_n$ sur la composante $AC$ de $A$. En particulier, toutes les composantes connexes par arcs de $SO(n)$ sont ouvertes.\\
Pour montrer qu'il n'existe qu'une seule composante, considérons $A\in SO(n)$. Puisque $A$ est une matrice orthogonale, il existe une matrice $P\in O(n)$ et des réels $\theta_1,\ldots,\theta_r$ tels que
\[
P^{\mathsf T}AP=\operatorname{diag}\bigl(R(\theta_1),\ldots,R(\theta_r),1,\ldots,1\bigr),
\]
où
\[
R(\theta)=\begin{pmatrix}\cos(\theta)&-\sin(\theta)\\\sin(\theta)&\cos(\theta)\end{pmatrix}.
\]
En effet, les éventuels blocs $(-1)$ apparaissent en nombre pair, car $\det(A)=1$, et peuvent donc être regroupés deux à deux sous la forme
\[
\begin{pmatrix}-1&0\\0&-1\end{pmatrix}=R(\pi).
\]
Définissons alors l'application
\[
\begin{aligned}
\gamma\colon[0,1]&\longrightarrow SO(n)\\
t&\longmapsto P\operatorname{diag}\bigl(R(t\theta_1),\ldots,R(t\theta_r),1,\ldots,1\bigr)P^{\mathsf T}
\end{aligned}
\]
Cette application est continue. De plus, chaque matrice $\gamma(t)$ est orthogonale et vérifie
\[
\det\bigl(\gamma(t)\bigr)=1,
\]
donc $\gamma(t)\in SO(n)$ pour tout $t\in[0,1]$. Enfin,
\[
\gamma(0)=I_n \qquad\text{et}\qquad \gamma(1)=A.
\]
Ainsi, toute matrice $A\in SO(n)$ appartient à la composante connexe par arcs $C$ de l'identité. On en déduit que
\[
C=SO(n).
\]
Par conséquent, $SO(n)$ ne possède qu'une seule composante connexe par arcs.\\
Et \underline{$SO(n)$ est connexe par arcs}.
\end{proof}
...
L'application exponentielle joue un rôle fondamental dans l'étude des groupes de Lie car elle établit un lien entre l'algèbre de Lie $\mathfrak{g}=T_{e}G$ et le groupe de Lie $G$.
...
Nous avons vu que, étant donné un vecteur $A\in\mathfrak{g}$, l'exponentielle permet de construire un élément $\exp(A)\in G$ (au moins dans le cas où $G=SO(n)$). L'exponentielle permet donc de passer d'une information infinitésimale, contenue dans l'espace tangent à l'identité, à une transformation appartenant au groupe. Dans le cas des rotations, elle associe à une matrice antisymétrique $A\in\mathfrak{so}(n)$ une matrice de rotation $e^A\in SO(n)$. Intéressons-nous au cas où $n=2$.
...
Dans ce chapitre, nous verrons comment cette équation différentielle linéaire apparaît naturellement par l'action du groupe spécial orthogonal $SO(n)$ sur des landmarks de $\mathbb{R}^n$, l'importance de ce résultat, ainsi que sa résolution. Commençons avec le groupe de Lie $SO(n)$.

\section{Obtenir l'équation différentielle \texorpdfstring{$\dot{x}(t)=Ax(t)$}{x.\ (t)=Ax (t)}}

Considérons des landmarks $x_i\in\mathbb{R}^n$ et l'action du groupe $SO(n)$ sur $\mathbb{R}^n$. Remarquons une égalité que l'on utilisera juste après.
\begin{remark}\label{rq:id}
On sait que $\forall X\in SO(n)\colon X^{\top}X=I_n$, et donc $X^{-1}=X^{\top}$.
\end{remark}

\subsubsection{Trouvons l'équation.}
Considérons une courbe différentiable à valeurs matricielles $M_t \in SO(n)$ et définissons la déformation d'un landmark $x_i$ par
\[
x_i(t)=M_{t}x_i.
\]
Différencions cette expression par rapport au temps,
\[
\dot{x}_i(t)=\dot{M}_{t}x_i.
\]
En posant $A(t) = \dot{M}_t{M_t}^{-1}$, on a $\dot{M}_t=A(t)M_t$. Montrons que $A(t)$ est antisymétrique: en dérivant l'identité~(\ref{rq:id}) ${M_t}^{\top}M_t=I_n$ par rapport au temps,
\[
{\dot{M}_t}^{\top}M_t+{M_t}^{\top}\dot{M}_t=0.
\]
Comme ${M_t}^{-1}={M_t}^{\top}$, on a $A(t)=\dot{M}_t{M_t}^{\top}$, et l'égalité ci-dessus donne $A{(t)}^{\top}+A(t)=0$. Ainsi $A(t) \in T_{I_n}SO(n) = \mathfrak{so}(n)$ (l'algèbre de Lie de $SO(n)$).\\
On obtient alors
\[
\begin{aligned}
\dot{x}_i(t)&=A(t)M_{t}x_i\\
\dot{x}_i(t)&=A(t)x_i(t).
\end{aligned}
\]
Donc si on suppose que ce vecteur est constant en fonction du temps $A(t)=A$, on retrouve
\[
\dot{x}_i(t)=Ax_i(t).
\]

\begin{remark}
Cette construction peut également s'interpréter à l'aide de la translation à droite sur $SO(n)$. Pour $M\in SO(n)$:
\[
\begin{aligned}
R_M\colon SO(n) &\longrightarrow SO(n)\\
X&\longmapsto XM
\end{aligned}
\]
Sa différentielle en $X\in SO(n)$ est
\[
\begin{aligned}
...
\end{aligned}
\]
En effet, pour $H\in \mathcal{M}_n(\mathbb{R})$,
\[
\begin{aligned}
R_M(X+H)&=(X+H)M\\
&=XM+HM\\
...
\begin{remark}
La solution de l'équation différentielle linéaire ci-dessus permet, à partir d'un seul élément $A$ de l'algèbre de Lie $\mathfrak{so}(n)$, de créer un mouvement sur tout le groupe $SO(n)$ et de déformer les landmarks par l'action de ce mouvement.
\end{remark}
...
\]
Or l'exponentielle d'une matrice de $\mathfrak{so}(n)$ est à valeur dans $SO(n)$~(\ref{prop:ensemble-exp}).\\
Contrairement au cas de $SO(2)$, on n'utilisera pas une écriture simple de $e^{tA}$. On va plutôt calculer avec les séries entières.\\
Notons
\[
...
\begin{figure}[H]
  ...
  \caption{Première et $50^{\text{e}}$ étape du recalage sur les rotations $SO(2)$.}\label{fig:RecalageRotationSO2}
\end{figure}
\end{exemple}
...
Pour l'équation sur $p$, on dérive le Hamiltonien réduit $H_r(q,p)=H(q,p,A^*(q,p))$ par rapport à $q$. Comme $A^*(q,p)$ maximise $H(q,p,\cdot)$ sur $\mathfrak{so}(n)$, la contribution de la dérivée de $A^*$ par rapport à $q$ disparait et il reste donc seulement $H$ exprimé en fonction de $q$:
\[
...
\end{figure}
L'approche scalaire est ici environ $1{,}6$ fois plus rapide que l'approche hamiltonienne sur cette expérience particulière. Cet écart s'explique par le nombre de variables intégrées à chaque pas de temps: l'approche directe ne propage que la position $x_t$, tandis que l'approche hamiltonienne propage simultanément la position $x_t$ \emph{et} le moment $p_t$, ce qui double la quantité de calcul à chaque itération d'Euler tout en recalculant $A^*_t$ à partir des deux tenseurs courants. En contrepartie, rappelons que l'approche hamiltonienne optimise un vecteur $p_0$ de même dimension que les nuages de points, ce qui la rend directement généralisable à des dynamiques bien plus riches (comme $SO(3)$ ou le cadre LDDMM général avec les difféomorphismes en dimension infinie), là où l'approche scalaire directe repose spécifiquement sur la paramétrisation par un angle $\omega$ propre à $SO(2)$.
\end{exemple}
...
Plusieurs prolongements naturels sont possibles, notamment l'abandon de la limitation aux rotations afin de se rapprocher du modèle LDDMM complet, ou encore l'extension aux formes plus riches que les simples landmarks. Au-delà de ces perspectives, ce stage a surtout constitué une occasion de mettre en pratique les outils d'algèbre et d'équa-diff, vus en cours théorique, dans un contexte appliqué. Il a aussi ouvert des portes sur des domaines des mathématiques pas étudiés en $2^{\text{e}}$ année de licence.
