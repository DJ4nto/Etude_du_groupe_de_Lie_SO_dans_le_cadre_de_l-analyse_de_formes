...
Ce rapport contient le travail réalisé pendant un stage consacré à l'étude mathématique et numérique d'un problème de recalage de formes. Le recalage consiste à déterminer une transformation permettant d'aligner une forme dite source sur une forme dite cible. Ce type de problème apparaît dans de nombreux domaines, par exemple en imagerie médicale ou encore en analyse morphologique, lorsqu'il s'agit de comparer des objets géométriques tout en tenant compte de leurs déformations possibles.

Le cadre général étudié durant ce stage est celui du \emph{Large Deformation Diffeomorphic Metric Mapping}, ou LDDMM\@. Cette approche sera détaillée dans le \textbf{Chapitre~\ref{chap:LDDMM}}. L'intérêt principal de cette méthode est de garantir que les transformations obtenues préservent la structure topologique des objets étudiés: il n'y a ni déchirure ni apparition de nouvelles formes.

Cependant, le modèle LDDMM complet est mathématiquement et numériquement riche. Dans ce stage, nous avons choisi d'en étudier une restriction plus simple: le cas où les transformations admissibles sont uniquement des rotations. Cette restriction conduit naturellement à l'étude du groupe spécial orthogonal $SO(n)$ et de son algèbre de Lie $\mathfrak{so}(n)$. Elle permet de comprendre, dans un cadre en dimension finie, comment une transformation continue peut être engendrée par une équation différentielle.

L'objectif du rapport est de présenter les outils théoriques nécessaires: groupes de Lie, algèbres de Lie, exponentielle de matrice et équations différentielles linéaires. Aussi, il s'agit d'illustrer ces notions par une implémentation numérique en Python, appliquée à des exemples simples de recalage par rotation dans le plan.

Le rapport suit naturellement la progression adoptée pendant le stage. Le premier chapitre rappelle les grandes idées du modèle LDDMM et introduit le problème variationnel de recalage. Le deuxième chapitre est consacré au groupe $SO(n)$, à ses propriétés géométriques et topologiques, ainsi qu'à son algèbre de Lie. Le troisième chapitre montre comment l'équation différentielle $\dot{x}(t)=Ax(t)$ apparaît naturellement dans ce contexte et comment elle se résout à l'aide de l'exponentielle de matrice. Le quatrième chapitre donne des précisions sur la formulation Hamiltonienne du problème de recalage pour les rotations. Le cinquième chapitre détaille le cas particulier de $SO(2)$ et $SO(3)$, qui permet d'obtenir une description explicite des rotations. Enfin, le dernier chapitre présente l'implémentation algorithmique et discute les résultats obtenus suite à l'exécution de ces algorithmes.
...
Voyons un exemple de recalage rigide avec une forme source $x_S$ et une autre forme cible $x_T$ séparées par une rotation (cet exemple~(\ref{ex:recalage-lecocq}) sera détaillé plus loin). La forme source, à laquelle on a appliqué une rotation, recouvre parfaitement la forme cible:
...
\begin{remark}[Espace tangent de $Q$]\leavevmode\\
...
\begin{definition}[Espace cotangent]\leavevmode\\
...
\]
L'élément $p\in T_q^*Q$ est appelé un covecteur. Dans le cadre hamiltonien, il correspond à un moment.
\end{definition}
...
\begin{theorem}
Notre équation ci-dessus admet un minimum.
\end{theorem}

\begin{proof}
Admis.\\
Nous ne démontrons pas ce résultat ici. Piste de preuve: On utilise une méthode de calcul variationnel~\cite{MP25}, ``Direct method in the calculus of variations'' pour montrer que l'équation de minimisation admet un minimum.
\end{proof}

Dans une écriture plus géométrique, on note $q_t\in Q$ la forme à l'instant $t$. L'action du champ de vecteurs $v_t$ sur la forme est notée
\[
v_t\circ q_t.
\]
...
\]
Le problème variationnel peut donc aussi s'écrire sous la forme
\[
\begin{aligned}
...
Ici, $q_S$ désigne la forme source et $q_T$ la forme cible. Le terme
\[
\int_0^1 \|v_t\|_V^2\,dt
\]
mesure l'énergie de la déformation, tandis que le terme
\[
\|q_1-q_T\|
\]
mesure l'écart final entre la forme transportée $q_1$ et la cible $q_T$.

\begin{remark}
Pour appliquer la méthode LDDMM, on n'a besoin que de l'action du groupe des difféomorphismes sur l'espace de formes. La formulation du problème de minimisation repose uniquement sur l'application du groupe à cet espace via $I_0 \mapsto I_0 \circ \varphi^{-1}$.
\end{remark}
...
\dot q_t = v_t\circ q_t.
\]
Pour traiter ce problème contraint, on introduit un covecteur $p_t\in T_{q_t}^*Q$, appelé moment. Le Hamiltonien de contrôle est
\[
...
\partial_v H(q,p,v)=0.
\]
On obtient ainsi une vitesse optimale
\[
v^*=v^*(q,p).
\]
...
H_r(q,p)=H(q,p,v^*(q,p)).
\]
Cette réduction permet de transformer le problème de contrôle optimal en un système d'équations différentielles sur $T^*Q$.

\subsection{Équations géodésiques}
Les trajectoires optimales, aussi appelées géodésiques, sont alors données par les équations géodésiques (appelées aussi équations hamiltoniennes).
\[
...
\begin{remark}
L'intérêt principal de la formulation hamiltonienne est qu'elle permet de remplacer l'optimisation sur le champ de vitesse $v_t$, qui appartient à un espace de dimension infinie, par l'étude d'un système différentiel portant sur les variables $(q_t,p_t)$. Dans le cas des points repères, ces variables sont en nombre fini: les positions $q_i(t)$ et les moments $p_i(t)$. Ainsi, le problème LDDMM se ramène à un problème en dimension finie, plus simple à analyser et à résoudre numériquement.
\end{remark}

Ce chapitre a posé les fondements nécessaires à l'étude des cas particuliers, comme celui des rotations $SO(2)$ et $SO(3)$, qui seront abordés dans les chapitres suivants. En effet, dans ce rapport nous ne traitons pas le modèle LDDMM complet. Nous étudions une restriction de ce cadre au cas où les transformations sont uniquement des rotations en dimension finie. Cette restriction permet d'obtenir un modèle plus simple que le modèle LDDMM complet qui repose sur l'action du groupe des difféomorphismes en dimension infinie.
...
Le cas pour $n=2$ est trivial, posons $n\geq3$.\\
...
(2) $SO(n)$ \textbf{borné}.\\
Soit $A\in SO(n)$, alors $\|A\|_F^2=\operatorname{Tr}(A^{\top}A)=\operatorname{Tr}(Id)=n$.\\
...
\begin{proposition}
$T_{Id}SO(n)=\{H\in \mathcal{M}_n(\mathbb{R})~|~H^{\top}=-H\}$ est l'algèbre de Lie de $SO(n)$. Plus précisément: $\mathfrak{so}(n)=T_{Id}SO(n)$.
\end{proposition}
...
\[
Df(Id)H=I_n^{\top}H+H^{\top}I_n=H+H^{\top}.
\]
Enfin,
\[
T_{Id}SO(n)=\ker(Df(Id))=\{H\in\mathcal{M}_n(\mathbb{R})~|~H^{\top}=-H\}
\]
...
L'espace $T_{M}SO(n)$ est isomorphe à $T_{Id}SO(n)$ pour tout $M\in SO(n)$
\end{proposition}
...
\end{aligned}
\]
Cette application est lisse~(\ref{prop:translation-lisse}), et son inverse est $R_{M^{-1}}$. Ainsi, $R_M$ est un difféomorphisme de $SO(n)$.\\
En particulier, sa différentielle en tout point est un isomorphisme linéaire. En évaluant en l'identité, on obtient
\[
T_{Id}R_M : T_{Id}SO(n) \longrightarrow T_M SO(n),
\]
qui est un isomorphisme.
Son inverse est donné par
\[
T_M R_{M^{-1}} : T_M SO(n) \longrightarrow T_{Id}SO(n),
\]
et par théorème de dérivation des fonctions composées, on a
\[
T_M R_{M^{-1}} \circ T_{Id}R_M=T_{Id}(R_{M^{-1}} \circ R_M)=\mathrm{Id}_{T_{Id}SO(n)}.
\]
On en déduit que $T_M SO(n)$ et $T_{Id}SO(n)$ sont isomorphes.
\end{proof}
...
Avant de nous restreindre aux cas explicites $SO(2)$ et $SO(3)$, reprenons le cadre hamiltonien introduit au \textbf{Chapitre~\ref{chap:LDDMM}}. Étudions le cas où le groupe de transformations est $SO(n)$ et que la forme étudiée est un ensemble de points repères.


\section{Espaces tangent et cotangent des points repères}
Dans ce cadre, le champ de vecteurs $v_t$ n'est plus un difféomorphisme quelconque: il est engendré par une matrice antisymétrique. On travaille donc dans l'espace
\[
V=Skew_n=\mathfrak{so}(n),
\]
où $Skew_n$ désigne l'espace des matrices antisymétriques de taille $n\geq 2$.\\
La forme étudiée est un ensemble de $N$ points repères. L'espace des formes est
\[
Q={(\mathbb{R}^n)}^N,
\]
...
\]
\begin{remark}
Cette identification de $T_{q}^*Q$ avec $T_{q}Q$ est spécifique au cas des points repères, où l'espace des formes est de dimension finie. C'est précisément cette propriété qui rend le problème plus simple que le modèle LDDMM complet, dans lequel l'espace des formes est de dimension infinie.
\end{remark}


\section{Hamiltonien sur \texorpdfstring{$SO(n)$}{SO (n)}}
Reprenons le Hamiltonien de contrôle introduit au \textbf{Chapitre~\ref{chap:LDDMM}}. Le contrôle est ici la matrice antisymétrique $A\in V=Skew_n$, et l'action du champ de vecteurs sur la forme est donnée par la multiplication matricielle $A\cdot q=Aq$. Le Hamiltonien s'écrit
\[
\begin{aligned}
...
\]
Or \(A\) est contraint d'appartenir à \(Skew_n\). La condition s'écrit
\[
\langle pq^\top-A,H\rangle=0,\qquad \forall\,H\in Skew_n,
\]
c'est-à-dire
\[
...
H_r(q,p)=H(q,p,A^*).
\]
Le problème de contrôle optimal se ramène ainsi à un système d'équations différentielles sur $T^*Q$, avec pour seule condition la donnée du moment initial $p_0$:
\[
\begin{aligned}
...
\]
\begin{remark}
Sur le groupe des difféomorphismes, on aurait $G=\mathrm{Diff}(\mathbb{R}^n)$ et $\mathfrak{g}=C_0^k(\mathbb{R}^n,\mathbb{R}^n)$, un espace de dimension infinie. Ici, dans le cas des points repères, on a $p\in T^*Q$ avec $\dim(T^*Q)=\dim(Q)=nN$ fini. L'optimisation ne porte plus sur le champ de vitesse en dimension infinie, mais uniquement sur le moment initial $p_0$, un objet de dimension finie.
\end{remark}
...
Maintenant, regardons l'énergie cinétique dans le cas des rotations de dimension 2. Avec le produit matriciel en loi de composition, on retrouve naturellement
\[
v_t=A_t,~A\in\mathfrak{so}(2).
\]
...
\]
c'est-à-dire, il existe une matrice $P$ inversible et un réel $\theta$ vérifiant
\[
...
\chi(t)=\det(R-tI_3).
\]
C'est un polynôme de degré $3$, donc il admet au moins une racine réelle $\lambda$. Comme $R$ est une matrice orthogonale, toutes ses valeurs propres sont de module $1$, on obtient donc nécessairement $\lambda=1$.\\
...
\begin{proof}\label{lem:matrices-SO3}
...
On reprend maintenant la formulation hamiltonienne établie au \textbf{Chapitre~\ref{chap:Hamiltonien}} dans le cas de $N$ points repères pour faire un algorithme de résolution par \emph{tir géodésique} (\textit{geodesic shooting})~\cite{MTT06}, où la seule variable d'optimisation est le moment initial.

\begin{proposition}[Équations géodésiques pour les points repères sous $SO(n)$]\label{prop:geodesique-points-reperes}\leavevmode\\
...
\subsection{Méthode pour le recalage rigide sur \texorpdfstring{$SO(2)$}{SO (2)}}
...
\end{figure}
L'approche scalaire est ici environ $1{,}6$ fois plus rapide que l'approche hamiltonienne. Cet écart s'explique par le nombre de variables intégrées à chaque pas de temps: l'approche directe ne propage que la position $x_t$, tandis que l'approche hamiltonienne propage simultanément la position $x_t$ \emph{et} le moment $p_t$, ce qui double la quantité de calcul à chaque itération d'Euler tout en recalculant $A^*_t$ à partir des deux tenseurs courants. En contrepartie, rappelons que l'approche hamiltonienne optimise un vecteur $p_0$ de même dimension que les points repères, ce qui la rend directement généralisable à des dynamiques bien plus riches (comme $SO(3)$ ou le cadre LDDMM général avec les difféomorphismes en dimension infinie), là où l'approche scalaire directe repose spécifiquement sur la paramétrisation par un angle $\omega$ propre à $SO(2)$.
\end{exemple}
...
D'un point de vue algorithmique: les deux implémentations, l'approche directe et le tir géodésique, ont confirmé l'efficacité du modèle lorsque la transformation est une rotation rigide, en mettant aussi en évidence ses limites quand on se restraint à ces transformations ``simples''. Dès qu'interviennent des déformations non rigides, la restriction à $SO(n)$ ne permet plus de représenter correctement les transformations. Ce travail met ainsi en lumière un compromis entre la rapidité de l'approche directe et la généralité de l'approche hamiltonienne.

Plusieurs prolongements naturels sont possibles, notamment l'abandon de la limitation aux rotations afin de se rapprocher du modèle LDDMM complet, ou encore l'extension aux formes plus riches que les simples points repères. Au-delà de ces perspectives, ce stage a surtout constitué une occasion de mettre en pratique les outils d'algèbre et d'équa-diff, vu en cours théorique, dans un contexte appliqué. Il a aussi ouvert des portes sur des zones des mathématiques pas étudiées en $2^{\text{ième}}$ année de licence.
